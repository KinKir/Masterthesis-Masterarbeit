...
DrawAlgebra.prototype.draw_tables = function(_this, color1, color2) {
  ...
  if (_this.imgDataResult) {
    drawUtil.ctx.putImageData(_this.imgDataResult, resultField.x,
      resultField.y);
  }
...
DrawAlgebra.prototype.draw_choose_rect = function(tuple, tuplex, tupley, attr,
  ...
  drawUtil.zoom_circle(text[attrPos.posAttr], attrPos.x + 1, attrPos.y - 2,
    attrPos.width, drawUtil.tupleHeight + 4);
};
/**
...
DrawAlgebra.prototype.move_to_result = function(_this, imgData, color1, color2,
  ...
  if (i != -1) {
    ...
    let des2 = {
      x: resultField.x,
      y: resultField.y + _this.result_num * (drawUtil.tupleMargin +
...
DrawAlgebra.prototype.animIntersection = function(res, input1, input2,
  ...
  function move_and_delete() {
    if (_this.is_Same(_this.src_or_val,
      ...
      drawUtil.ctx.clearRect(0, drawUtil.animField.y,
        drawUtil.fullWidth, resultField.y);
      nextAnimation();
    }
  };
};
      ...
      function nextTuple() {
        ...
        if (_this.nth_tuple1 < _this.tuplesInput1.value.length) {
          ...
          drawUtil.ctx.clearRect(
            ...
            drawUtil.animField.y, drawUtil.fullWidth, resultField.y);
          drawUtil.ctx.clearRect(
